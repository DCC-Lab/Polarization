...
class BirefringentMaterial(JonesMatrix):
    def __init__(self, deltaIndex:float, fastAxisOrientation, physicalLength=0):
        ...
        self.jDeltaIndexTimesLength = 1j * deltaIndex*physicalLength
    ...
    def computeMatrix(self, k=None):
        if k is not None:
            D = exp(1j * self.deltaIndexTimesLength * k)
    ...
    def computePythonMatrix(self, k=None):
        if k is not None:
            D = exp(self.jDeltaIndexTimesLength * k)
            Ap = self.c2 + D*self.s2
            Bp = self.cs*D - self.cs 
            Dp = self.c2*D + self.s2
